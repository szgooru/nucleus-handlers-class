...
class JoinClassByStudentHandler implements DBHandler {
  ...
  public ExecutionResult<MessageResponse> validateRequest() {
    ...
    // Delegate it to authorizer to confirm if this is either an open class in which case allow user to join or it is restricted class but the user
    // is invited which is also fine. Else no authorization
    return AuthorizerBuilder.buildJoinClassByStudentAuthorizer(this.context, this.membership).authorize(this.entityClass);
  }
