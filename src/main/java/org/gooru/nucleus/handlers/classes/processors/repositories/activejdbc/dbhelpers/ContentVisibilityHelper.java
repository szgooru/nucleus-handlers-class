...
public final class ContentVisibilityHelper {
    ...
    public static JsonObject updateContentVisibility(JsonObject requestPayload, String existingVisibilityString) {
        JsonObject existingVisibility = existingVisibilityString != null && !existingVisibilityString.isEmpty()
            ? new JsonObject(existingVisibilityString) : null;
    ...
    private static final String UNITS_QUERY_FILTER =
        "course_id = ?::uuid and unit_id = ANY(?::uuid[]) and is_deleted = false";
    private static final String LESSONS_QUERY_FILTER =
        "course_id = ?::uuid and lesson_id = ANY(?::uuid[]) and is_deleted = false";
    private static final String COLLECTIONS_QUERY_FILTER =
            ...
            if (validateContentCountInCourse(units, courseId, TABLE_UNIT, UNITS_QUERY_FILTER)) {
                ...
                return new ExecutionResult<>(
                    MessageResponseFactory.createInvalidRequestResponse(
                        RESOURCE_BUNDLE.getString("content.visibility.unit.course.invalid")),
    ...
    private static ExecutionResult<MessageResponse> validateLessonsExistence(JsonArray lessons, String courseId) {
        try {
            if (validateContentCountInCourse(lessons, courseId, TABLE_LESSON, LESSONS_QUERY_FILTER)) {
                ...
                return new ExecutionResult<>(
                    MessageResponseFactory.createInvalidRequestResponse(
                        RESOURCE_BUNDLE.getString("content.visibility.lesson.course.invalid")),
                    ExecutionResult.ExecutionStatus.FAILED);
            }
        } catch (DBException dbe) {
    ...
    private static ExecutionResult<MessageResponse> validateCollectionsAssessmentsExistence(JsonArray collections,
        JsonArray assessments, String courseId) {
     ...
     */
    private static boolean validateContentCountInCourse(JsonArray contentIds, String courseId, String tableName,
        String dbQuery) {
        ...
        long count = Base.count(tableName, dbQuery, courseId,
            Utils.convertListToPostgresArrayStringRepresentation(contentIds.getList()));
